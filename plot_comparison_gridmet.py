#!/usr/bin/env python3
"""
Combined plotting script for Spatial CIMIS analysis (GridMET Grid Version).

Creates two types of plots:
1. Spatial comparison: 3-panel maps (Spatial CIMIS GridMET, GridMET, Bias)
...
- Uses GridMET-projected Spatial CIMIS data (*_gridmet.csv files)
- Both datasets use same CRS (EPSG:4326) for easier comparison
...
def create_spatial_comparison_plot_gridmet(config):
    """Create 3-panel spatial comparison plot (Spatial CIMIS GridMET, GridMET, Bias)."""
    print("\n" + "="*70)
    print("SPATIAL COMPARISON PLOT (GridMET Grid Version)")
    print("="*70)
    ...
    # Construct file paths for GridMET-projected data
    spatial_file = Path(output_path) / config.get('spatial_mean_output_gridmet', f'spatial_mean_{variable.lower()}_gridmet.nc')
    gridmet_file = Path(output_path) / config['gridmet_mean_output'].replace('{variable}', variable.lower())
    
    print(f"\nVariable: {variable}")
    print(f"Spatial CIMIS GridMET file: {spatial_file}")
    print(f"GridMET file: {gridmet_file}")
    print(f"Shapefile: {shapefile_path}")
    
    # Load spatial data (GridMET-projected)
    print("\n1. Loading Spatial CIMIS climatology (GridMET grid)...")
    ...
    # Set CRS for spatial data (both are now in EPSG:4326)
    print("\n4. Setting CRS for spatial data...")
    if 'lon' in spatial_data.dims and 'lat' in spatial_data.dims:
        spatial_data.rio.set_spatial_dims(x_dim='lon', y_dim='lat', inplace=True)
    elif 'x' in spatial_data.dims and 'y' in spatial_data.dims:
    ...
    print("  Clipping Spatial CIMIS (GridMET grid)...")
    # Extract individual geometries from the shapefile
    ca_geometries = ca_shape.geometry.values
    spatial_clipped = spatial_data.rio.clip(ca_geometries, ca_shape.crs, drop=True)
    ...
    # Reproject GridMET to match Spatial CIMIS grid (both are EPSG:4326 but may have different resolutions)
    ...
    # Panel 1: Spatial CIMIS (GridMET grid)
    if spatial_values is not None:
        ...
        axes[0].set_title('Spatial CIMIS\n(GridMET Grid)', fontsize=12, fontweight='bold')
...
def load_timeseries_data_gridmet(config):
    """Load station, spatial (GridMET grid), and GridMET time series data."""
    print("\nLoading time series data (GridMET grid version)...")
    
    output_path = config['output_path']
    variable = config['variable']
    
    # Construct file paths based on variable (using GridMET-projected files)
    station_file = Path(output_path) / f"station_{variable.lower()}_data.csv"
    spatial_file = Path(output_path) / f"spatial_cimis_station_{variable.lower()}_gridmet.csv"
    ...
    # Load Spatial CIMIS predictions (GridMET grid version)
    ...
    print(f"  Spatial CIMIS (GridMET grid): {spatial_data.shape}")
    ...
    for i, sn in enumerate(station_list):
        ...
        if sn_str in station_monthly.columns and sn_str in spatial_monthly.columns:
            ...
            ax[0].plot(spatial_monthly.index, spatial_monthly[sn_str].values,
                      color=COLORS[0], label='Spatial CIMIS\n(GridMET Grid)', linestyle='-',
                      marker='o', zorder=4)
...
def create_station_timeseries_plot_gridmet(config):
    """Create station time series comparison plot (GridMET grid version)."""
    print("\n" + "="*70)
    print("STATION TIME SERIES PLOT (GridMET Grid Version)")
    print("="*70)
    ...
    # Load data (using GridMET-projected files)
    station_data, spatial_data, gridmet_data = load_timeseries_data_gridmet(config)
    ...
    # Create plot
    output_file = Path(output_path) / f"{variable.lower()}_station_timeseries_gridmet.png"
...
def main():
    """Main function."""
    if len(sys.argv) < 2:
        print("Usage: python plot_comparison_gridmet.py <config_file>")
        sys.exit(1)
    
    config_file = sys.argv[1]
    
    print("="*70)
    print("SPATIAL CIMIS COMPARISON PLOTTING (GridMET Grid Version)")
    print("="*70)
    ...
    if create_spatial_plot:
        try:
            create_spatial_comparison_plot_gridmet(config)
    ...
    if create_timeseries_plot:
        try:
            create_station_timeseries_plot_gridmet(config)
        except Exception as e:
